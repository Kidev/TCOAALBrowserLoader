...
(function () {
    ...
    function isSaveKey(key) {
        ...
        return bare === 'RPG Global' || bare === 'RPG Config' ||
               /^RPG File\d+$/.test(bare);
    }

    function openSaveDb(callback) {
        if (_saveDb) { callback(_saveDb); return; }
        try {
    ...
    function restoreSavesFromIDB() {
        ...
        openSaveDb(function (db) {
            if (!db) { _savesRestored = true; return; }
            try {
                ...
                cursor.onsuccess = function (e) {
                    ...
                    if (c) {
                        ...
                        if (count > 0) {
                            console.log('[lang-shim] Restored ' + count + ' save(s) from IndexedDB');
                        }
                        _savesRestored = true;
                    }
                };
                cursor.onerror = function () { _savesRestored = true; };
            } catch (e) {
                _savesRestored = true;
            }
        });
    ...
    function isPluginType(type) { return type && type.indexOf('plugin') >= 0; }
    ...
    function openAssetsDb(callback) {
        if (_assetsDb) { callback(_assetsDb); return; }
        try {
    ...
    function deleteAssetsByPrefix(db, prefix, callback) {
        try {
            ...
            cur.onerror = function () { if (callback) callback(0); };
    ...
    var _activeMod = null;
    try { _activeMod = localStorage.getItem('_activeMod') || null; } catch (e) {}
    ...
    function setActiveMod(modId, onDone) {
        ...
        if (navigator.serviceWorker && navigator.serviceWorker.controller) {
            navigator.serviceWorker.controller.postMessage({
                type: 'setActiveMod', id: modId || null
            });
        }
    ...
    function getActiveMod() { return _activeMod; }
    ...
    function loadPluginMod(pluginId, callback) {
        ...
        if (!allFiles) { if (callback) callback(); return; }
        ...
        if (jsFiles.length === 0) { if (callback) callback(); return; }
    ...
    if (navigator.serviceWorker) {
        navigator.serviceWorker.ready.then(function () {
            if (navigator.serviceWorker.controller) {
                navigator.serviceWorker.controller.postMessage({
                    type: 'setActiveMod', id: _activeMod || null
                });
            }
        });
    }
            ...
            try {
                ...
                if (parsed && (parsed.linesLUT || parsed.labelLUT)) {
                    putAsset(db, '__mod_lang_data__:' + modId, json, function () {
                        console.log('[lang-shim] Cached lang data for mod ' + modId +
                            ' (' + json.length + ' chars)');
                        callback();
                    });
                    return;
    ...
    function installMod(modId, basePath, onProgress, onDone, onError) {
        if (!basePath) { if (onError) onError('No mod path configured'); return; }
        ...
        openAssetsDb(function (db) {
            if (!db) { if (onError) onError('Cannot open IndexedDB'); return; }
            ...
            function processBatch() {
                ...
                for (var i = 0; i < batch.length; i++) {
                    (function (relPath) {
                        fetch(wwwBase + relPath).then(function (res) {
                            ...
                            putAsset(db, 'mod:' + modId + ':' + relPath, buf, function () {
                                ...
                                if (stored % 20 === 0 || stored === total) {
                                    onProgress({ percent: pct, message: 'Installing... ' + stored + '/' + total });
                                }
                                pending--;
    ...
    function uninstallMod(modId, callback) {
        openAssetsDb(function (db) {
            if (!db) { if (callback) callback('Cannot open IndexedDB'); return; }
    ...
    function applyPatches() {
            ...
            DataManager.latestSavefileId = function () {
                ...
                if (globalInfo) {
                    for (var i = 1; i < globalInfo.length; i++) {
                        if (globalInfo[i] && this.isThisGameFile(i) &&
        ...
        if (typeof ConfigManager !== 'undefined' && typeof Graphics !== 'undefined') {
        ...
        if (typeof Window_TitleCommand !== 'undefined' && getModList().length > 0) {
            ...
            Window_TitleCommand.prototype.makeCommandList = function () {
                ...
                var modsCmd = { name: 'Mods', symbol: 'mods', enabled: true, ext: null };
                if (quitIdx >= 0) {
        ...
        Scene_Mods.prototype.update = function () {
            ...
            if (this._listWindow && this._listWindow.active && Input.isTriggered('delete')) {
        ...
        Scene_Mods.prototype.onModOk = function () {
            var mod = this._listWindow.selectedMod();
            if (!mod) { this._listWindow.activate(); return; }

            var status = _modStatus[mod.key];
            var installed = status && status.installed;

            if (!installed) {
                if (this._installing) { this._listWindow.activate(); return; }
        ...
        Scene_Mods.prototype.onConfirmYes = function () {
            ...
            if (!action) { this._listWindow.activate(); return; }
            ...
            switch (action.type) {
                case 'install':
                    ...
                    installMod(mod.key, mod.path,
                        function onProgress(d) {
                            _modStatus[mod.key]._progress = d.message || ('Installing... ' + d.percent + '%');
                            self._listWindow.refresh();
                        },
                        function onDone(d) {
        ...
        Window_ModConfirm.prototype.windowWidth = function () { return 360; };
        ...
        Window_ModList.prototype._loadIcons = function () {
            ...
            for (var i = 0; i < this._mods.length; i++) {
                ...
                if (mod.icon) {
                    ...
                    bmp.addLoadListener(function () { self.refresh(); });
                }
            }
            var defIcon = getDefaultModIcon();
            if (defIcon) {
                defIcon.addLoadListener(function () { self.refresh(); });
            }
        };
        ...
        Window_ModList.prototype.maxVisibleItems = function () { return 5; };
        ...
        Window_ModList.prototype.drawItem = function (index) {
            ...
            var src = (iconBmp && iconBmp.isReady() && iconBmp.width > 1)
                ? iconBmp : getDefaultModIcon();
            ...
            var typeLabel = '[' + (mod.type || 'overhaul').replace(/\b\w/g, function (c) {
            ...
            if (isBuiltIn(mod)) {
                ...
                this.drawText(active ? 'Enabled' : 'Disabled',
                              textX, lineY + smallLine, availW);
            } else if (status && status._downloading) {
                this.contents.textColor = '#ffff88';
                this.drawText(status._progress || 'Installing...', textX, lineY, availW);
                ...
                this.drawText(installed ? 'Installed' : 'Not installed', textX, lineY, availW);
                if (installed) {
                    var isActive;
                    if (isPluginType(mod.type)) {
                    ...
                    this.drawText(isActive ? 'Enabled' : 'Disabled',
                                  textX, lineY + smallLine, availW);
                }
            }
            this.resetTextColor();
            ...
            if (mod.lastUpdate && !isBuiltIn(mod)) {
                this.contents.fontSize = 18;
                this.drawText(mod.lastUpdate, rect.x, rect.y + rect.height - lineHeight - 2,
                              rect.width, 'right');
                this.contents.fontSize = this.standardFontSize();
            }
        };
        ...
        console.log('[lang-shim] Mod system patches applied' +
