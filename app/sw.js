...
function mimeFor(filePath) {
    ...
    return MIME_MAP[(m && m[0].toLowerCase()) || ''] || 'application/octet-stream';
}
...
function loadDrmFromIdb(db) {
    return new Promise(resolve => {
        const req = db.transaction(STORE_NAME, 'readonly')
            .objectStore(STORE_NAME).get(DRM_CACHE_KEY);
        req.onsuccess = () => {
...
async function loadBaseCLD(db) {
    ...
    if (cldRaw !== null) {
        ...
        if (sig === 'LANGDATA') {
            try { return JSON.parse(new TextDecoder().decode(bytes.slice(8))); } catch {}
        }
    }
...
async function loadModLangData(db, modId) {
    ...
    if (locRaw !== null) {
        try {
            const text = typeof locRaw === 'string' ? locRaw
                : new TextDecoder().decode(locRaw);
...
const DRM_PLUGIN_FILES = [
    ['js/plugins/YEP_SaveEventLocations.js', ['_0xabd953_', '_0xb1c2fa_', '_0x32d8dd_']],
...
const DRM_ORDER = [
    '_0xabd953_', '_0xb1c2fa_', '_0x82f7bc_', '_0x32d8dd_',
    '_0x2f7cd8_', '_0xf3e01e_', '_0x87159a_', '_0x27f7fc_',
];
    ...
    for (const [logicalPath, fnNames] of DRM_PLUGIN_FILES) {
        ...
        for (const key of [hashed, logicalPath]) {
            ...
            if (raw !== null) {
                ...
                text = decoder.decode(decrypted instanceof ArrayBuffer ? decrypted : decrypted);
                break;
            }
        }
        ...
        for (const fnName of fnNames) {
            const m = text.match(
                new RegExp(`function ${fnName}\\(\\)\\s*\\{\\s*return\\s*"([^"]+)"\\s*;\\s*\\}`)
            );
            if (!m) return null;
...
self.addEventListener('fetch', event => {
    ...
    let logicalPath;
    try {
        logicalPath = decodeURIComponent(url.pathname.replace(/^\/+/, '')) || 'index.html';
    } catch {
    ...
    if (logicalPath === 'loader.html' ||
        ...
        logicalPath === 'favicon.ico') return;
...
async function serveFromIDB(logicalPath, request) {
    ...
    if (logicalPath === 'lang-data.json') {
        ...
        console.warn('[sw] lang-data.json: no data found, falling through to network');
        return fetch(request);
    }
    ...
    if (!logicalPath.match(/\.[a-z0-9]+$/i)) {
        const GUESS = { data: ['.json'], img: ['.png'], audio: ['.ogg', '.m4a'], movies: ['.webm'] };
